...
void hittingSound()
{
	static sf::SoundBuffer buffer("sounds/hitting.wav");   // Loaded once
	static sf::Sound sound(buffer);                         // Bound once

	sound.play();    // Plays asynchronously (no Sleep needed)
}

void missingSound()
{
	static sf::SoundBuffer buffer("sounds/missing.wav"); // loads once
...
void destroySound()
{
	static sf::SoundBuffer buffer("sounds/boatdestroyed.wav");
	static sf::Sound sound(buffer);
...
void winningSound()
{
	static sf::SoundBuffer buffer("sounds/winning.wav");
	static sf::Sound sound(buffer);
...
void playersMap(
	...
	sf::RectangleShape tileMap[16][16],
	sf::RectangleShape tileMapPlayer2[34][16],
	int mapSize,
	int mapSizePlayer2,
	int initialStarting,
	float gridSizeF)
{
	...
	for (int x = 0; x < mapSize; x++)
	{
		for (int y = 0; y < mapSize; y++)
		{
			...
			tileMap[x][y].setPosition(sf::Vector2f(gridSizeF * x, gridSizeF * y));
		}
	}
	...
	for (int x = initialStarting; x < mapSizePlayer2; x++)
	{
		for (int y = 0; y < mapSize; y++)
		{
			tileMapPlayer2[x][y].setSize(sf::Vector2f(gridSizeF, gridSizeF));
			tileMapPlayer2[x][y].setFillColor(sf::Color::White);
			tileMapPlayer2[x][y].setOutlineThickness(1.f);
			tileMapPlayer2[x][y].setOutlineColor(sf::Color::Black);
			tileMapPlayer2[x][y].setPosition(sf::Vector2f(gridSizeF * x, gridSizeF * y));
		}
	}
}
...
void rotatingFunc(const sf::Event& event, Vector2u& mousePosGrid, RectangleShape tileMap[16][16], RectangleShape tileMapPlayer2[34][16], RectangleShape& shipDeployer, char arr1[16][16], char arr2[16][16],bool& inputCheck,bool& ship, float& gridSizeF, bool& player1, bool& player2, bool& rotateFlag,int shipSize, int mapSize,int mapSizePlayer2,bool &wait, bool& compPlay,int initialStarting, Vector2f randomPosShip, bool& compPlayCheck,bool &randomFlag) {
	if (wait == true) {
		...
		if (Keyboard::isKeyPressed(Keyboard::Key::R)) {
			...
			if (!inputCheck && player1 && posShip.x + shipSize <= mapSize)
			{
				shipDeployer.setSize(sf::Vector2f(gridSizeF * shipSize, gridSizeF));
				shipDeployer.setPosition(sf::Vector2f(posShip.x * gridSizeF, posShip.y * gridSizeF));
				rotateFlag = true;
			}

			if (!inputCheck && player2 && posShip.x + shipSize <= mapSizePlayer2)
			{
				shipDeployer.setSize(sf::Vector2f(gridSizeF * shipSize, gridSizeF));
				shipDeployer.setPosition(sf::Vector2f(posShip.x * gridSizeF, posShip.y * gridSizeF));
				rotateFlag = true;
		...
		/////////////////Rotating Vertically//////////////////////
		if (sf::Mouse::isButtonPressed(sf::Mouse::Button::Left) || compPlay) {
			...
			if (compPlay) {
				...
				if (!inputCheck &&
					compPlayCheck &&
					randomPosShip.y + shipSize <= mapSize &&
					tileMapPlayer2[static_cast<int>(randomPosShip.x)]
					[static_cast<int>(randomPosShip.y)]
					.getFillColor() != sf::Color::Blue)
				{
					compPlayCheck = false;
					randomFlag = false;

					shipDeployer.setSize(sf::Vector2f(gridSizeF, gridSizeF * shipSize));
					shipDeployer.setPosition(sf::Vector2f(randomPosShip.x * gridSizeF,
						randomPosShip.y * gridSizeF));
				}
			...
			if (!inputCheck && player1 && posShip.y + shipSize <= mapSize)
			{
				shipDeployer.setSize(sf::Vector2f(gridSizeF, gridSizeF * shipSize));
				shipDeployer.setPosition(sf::Vector2f(posShip.x * gridSizeF,
					posShip.y * gridSizeF));
			}

			if (!inputCheck && player2 && posShip.y + shipSize <= mapSize)
			{
				shipDeployer.setSize(sf::Vector2f(gridSizeF, gridSizeF * shipSize));
				shipDeployer.setPosition(sf::Vector2f(posShip.x * gridSizeF,
					posShip.y * gridSizeF));
...
void nextSelect(const sf::Event& event, bool& inputCheck, bool& ship, int& shipNum, int shipSize,int &shipCountPlayer1, int& shipCountPlayer2,bool &player1,bool& player2,bool &wait,bool &compPlay,bool& compPlayCheck, bool& randomFlag) {
...
void easyMode(RenderWindow &window,RectangleShape& shipDeployer, RectangleShape& tileSelector, Vector2f& posShip, Vector2i &mousePosScreen, Vector2i &mousePosWindow, Vector2u &mousePosGrid, Vector2f& mousePosView, bool& inputCheck, bool& ship, bool& player1, bool& player2, bool& axisCheck, float& gridSizeF, unsigned& gridSizeU,Text &text, Text& shipDeploy, char arr1[16][16], char arr2[16][16], RectangleShape tileMap[16][16], RectangleShape tileMapPlayer2[34][16], bool &easyMod, bool& mediumMod , bool& hardMod) {
	...
	{	
		...
		{
			const sf::Event& event = *maybeEvent; // use const ref
			...
			if (compOutput) {
				...
				if (!font.openFromFile("Fonts/SourceSansPro-SemiBold.ttf")) {
					// Handle error
				...
				comp.setFont(font);
				comp.setPosition(sf::Vector2f(400.f, 400.f)); 
				...
				if (sf::Keyboard::isKeyPressed(sf::Keyboard::Key::Y)) {
					...
					furtherOutput = true;
					window.clear();
					sf::sleep(sf::milliseconds(250)); // SFML sleep instead of Sleep
				}
					...
					compOutput = false;
					window.clear();
					sf::sleep(sf::milliseconds(250)); // SFML sleep
				}
...
int main()
{
	...
	text.setFont(font);
	text.setPosition(sf::Vector2f(650 + 120, 360 + 100));
	text.setOrigin(sf::Vector2f(650 / 2.0f, 360 / 2.0f));


	Text shipDeploy(font);
	shipDeploy.setCharacterSize(20);
	shipDeploy.setFillColor(sf::Color::White);
	shipDeploy.setFont(font);
	shipDeploy.setPosition(sf::Vector2f(1 * gridSizeF, 10 * gridSizeF));
	...
	gameName.setPosition(sf::Vector2f(220.f, 1.f));
	...
	mainText.setPosition(sf::Vector2f(560.f, 230.f));
	mainText.setString("Main Menu");


	Text mainOptions(font);
	...
	mainOptions.setPosition(sf::Vector2f(560.f, 350.f));
	mainOptions.setString("1. Easy");

	Text mainOptionsMedium(font);
	...
	mainOptionsMedium.setPosition(sf::Vector2f(560.f, 400.f));
	mainOptionsMedium.setString("2. Medium");

	Text mainOptionsHard(font);
	...
	mainOptionsHard.setPosition(sf::Vector2f(560.f, 450.f));
	mainOptionsHard.setString("3. Hard");

	Text scoresPlayer(font);
	...
	scoresPlayer.setPosition(sf::Vector2f(560.f, 500.f));
	scoresPlayer.setString("4. Scores");

	Text scoreShow(font);	Text scoreShow2(font);


	bool player1, player2=false, ship = false, inputCheck = true, axisCheck = false;

	//Init window
	sf::RenderWindow window(sf::VideoMode(sf::Vector2u(1920, 1080)), "BattelShip");
	window.setFramerateLimit(120);

	sf::RenderWindow mainMenu(sf::VideoMode(sf::Vector2u(1450, 720)), "BattelShip | Main Menu");
	mainMenu.setFramerateLimit(120);
	...
	while (mainMenu.isOpen())
	{
		...
		while (const std::optional event = mainMenu.pollEvent())
		{
			...
			if (sf::Keyboard::isKeyPressed(sf::Keyboard::Scancode::Num1)) {
				...
				mediumMod = true;
				shipDeploy.setPosition(sf::Vector2f(1.f * gridSizeF, 14.f * gridSizeF));
				mainMenu.close();
			}
			else if (sf::Keyboard::isKeyPressed(sf::Keyboard::Scancode::Num3)) {
				hardMod = true;
				shipDeploy.setPosition(sf::Vector2f(1.f * gridSizeF, 17.f * gridSizeF));
				mainMenu.close();
			}
				...
				scoreShow.setPosition(sf::Vector2f(20.f, 50.f));
				...
				scoreShow2.setPosition(sf::Vector2f(20.f, 100.f));
